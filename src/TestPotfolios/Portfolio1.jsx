import React from 'react'
import { AiFillInstagram ,AiFillGithub ,AiFillFacebook ,AiFillAndroid} from "react-icons/ai"
import {GoCodeReview} from 'react-icons/go'
import {BsLinkedin} from 'react-icons/bs'
import {ImWhatsapp} from 'react-icons/im'
import { MdOutlineEmail} from 'react-icons/md'
import ImgMain from "../image/img_main.png"
import Footer from "../component/Footer"

function Portfolio1() {
  return (
    <div className=' bg-slate-900 p-4'>
       <div className=''>
        <div>
          <div className='w-full text-white text-xl flex justify-end items-center p-4 px-12 fixed top-0 right-0 bg-slate-950'>
          <a href='#home' className='px-2 mx-3  hover:text-gray-300'>Home</a>
          <a href='#project' className='px-2 mx-3  hover:text-gray-300'>Project</a>
          <a href='#contact' className='px-2 mx-3  hover:text-gray-300'>Contact</a>
         <a href=''> <AiFillFacebook size={20} className='mx-2 hidden md:flex'/></a>  
         <a href=''> <AiFillInstagram size={20} className='mx-2 hidden md:flex'/></a>
         <a href='https://github.com/rachidcooder'><AiFillGithub size={20} className='mx-2 hidden md:flex'/></a>
          </div>
          <div className=' flex items-center  p-5 flex-col md:flex-row  md:justify-between py-12 ' id='home'>
            <div className='text-gray-50 p-5'>
              <h1 className='text-3xl text-gray-200 py-2  font-bold'>Hi ,I'am Andresal</h1>
              <h2 className='text-2xl py-2'>I'm Full Stack Developer</h2>
              <p className='text-gray-400'>
                    Lorem ipsum dolor sit, amet consectetur adipisicing</p> 
              <p className='text-gray-400'>
                    porro soluta quibusdam sunt beatae fugiat corporis,</p> 
             <p className='text-gray-400 mb-5'>
                    ullam fuga similique facilis veritatis eum consequuntur.</p> 
              <a href='#project' className='px-2 py-1  border border-white rounded-lg my-3 border-spacing-2 text-gray-100'
                >View My Project</a>
            </div>
            <img className=' object-cover bg-transparent rounded-lg h-[600px] w-[500px]'
             src={ImgMain} alt='' />
          </div>
        </div>
   <div className=' '>
    {/**My Skill */}
      <h1 className='text-center text-3xl text-gray-100 p-4 font-bold'>My Skills</h1>
     <div>
       <h2 className='px-5 py-2 bg-slate-100 text-gray-800 rounded-lg md:rounded-full  text-xl mx-5 m-2'
          ><span className='text-black font-bold'>Front-end : </span>I have a strong command of front-end technologies,
               (Html, Css,JavaScript,React Js,Vue.js ,Tailwind)</h2>
         <h2 className='px-5 py-2 bg-slate-100 text-gray-800 rounded-lg md:rounded-full  text-xl mx-5 m-2 text-center'
          ><span className='text-black font-bold'>Back-end : </span>
          I excel in creating robust APIs and managing databases using technologies like Node.js & ExpressJs and MongoDB  
          </h2> 
          <h2 className='px-5 py-2 bg-slate-100 text-gray-800 rounded-lg md:rounded-full  text-xl mx-5 m-2 '
          ><span className='text-black font-bold'>Programing Languages : </span> JavaScript , Java ,C ,C++  
          </h2> 
     </div>
   </div>
   {/**My Projects */}
   <div>
    <h1 id='project' className='text-3xl font-bold text-center text-white p-3'>My Project</h1>
   </div>
    <div className='grid gap-6 p-5 lg:grid-cols-4 md:grid-cols-2'>
    
      <div className=' bg-slate-100 shadow-lg rounded-lg flex flex-col '>
        <img className='h-[200px] rounded-lg'
          src='https://images.pexels.com/photos/1226398/pexels-photo-1226398.jpeg?auto=compress&cs=tinysrgb&w=300' alt=''/>
         <h1 className ="text-3xl text-center py-2 font-bold text-gray-800">Task Manger</h1>
         <p className="p-3" >
          Lorem ipsum dolor sit amet consectetur, adipisicing elit. 
         </p>
      </div>

        <div className=' bg-slate-100 shadow-lg rounded-lg flex flex-col '>
        <img className='h-[200px] rounded-lg'
          src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSYLPy2WZD5ReCyn1h3YSS50kuhP0IZPpw9CA&usqp=CAU' alt=''/>
         <h1 className ="text-3xl text-center py-2 font-bold text-gray-800">Ecommerce Store</h1>
         <p className="p-3" >
          Lorem ipsum dolor sit amet consectetur, adipisicing elit. 
         </p>
      </div>

        <div className=' bg-slate-100 shadow-lg rounded-lg flex flex-col '>
        <img className='h-[200px] rounded-lg'
          src='data:image/png;base64,' alt=''/>
         <h1 className ="text-3xl text-center py-2 font-bold text-gray-800">Chat App</h1>
         <p className="p-3" >
          Lorem ipsum dolor sit amet consectetur, adipisicing elit. 
         </p>
      </div>

        <div className=' bg-slate-100 shadow-lg rounded-lg flex flex-col '>
        <img className='h-[200px] rounded-lg'
          src='https://images.pexels.com/photos/1226398/pexels-photo-1226398.jpeg?auto=compress&cs=tinysrgb&w=300' alt=''/>
         <h1 className ="text-3xl text-center py-2 font-bold text-gray-800">Task Manger</h1>
         <p className="p-3" >
          Lorem ipsum dolor sit amet consectetur, adipisicing elit. 
         </p>
      </div>
    
    </div>
     <div className='bg-white rounded-lg m-4 p-4'>
      <h1 className='p-2 text-center text-2xl font-bold'> Somme other Skills & Certifications </h1>
       <p className=' text-gray-900'><span className='text-xl font-bold'>Languages : </span>Amazigh, Arabic ,Frensh ,English</p>
       <p className=' text-gray-900'><span className='text-xl font-bold'>Dogree : </span>Math and Computer Sciense (License en Math & Informatique)</p>
       <p className=' text-gray-900'><span className='text-xl font-bold'>Work on team : </span>I was always working in team since when i start learning progamming  </p>
       <p className=' text-gray-900'><span className='text-xl font-bold'>git & github : </span>I have a strong command of Git and GitHub, facilitating efficient version control and collaborative development for projects. </p>
       <p className=' text-gray-900'><span className='text-xl font-bold'>Doker :: </span>"I frequently utilize Docker to containerize applications, ensuring consistent and reproducible environments for development and deployment.</p>
     </div>
    <h1 className='text-white font-bold text-center pt-5 text-3xl'>Contact Me </h1>
    <div className='flex justify-center items-center text-white p-5' id='contact'>
     <a href=''><AiFillFacebook size={30} className='mx-3 '/></a> 
     <a href=''> <AiFillInstagram size={30} className='mx-3 '/></a> 
     <a href='https://github.com/rachidcooder'> <AiFillGithub size={30} className='mx-3'/></a> 
     <a href=''> <BsLinkedin size={30} className='mx-3 '/></a> 
     <a href=''> <ImWhatsapp size={30} className='mx-3 '/></a>
     <a href='https://mail.google.com/mail/u/0/?tab=rm&ogbl#inbox'> <MdOutlineEmail size={30} className='mx-3'/></a>
    </div>
  </div>
     <div className='border-t-2 border-gray-300 flex flex:col md:flex-row md:justify-between py-5 text-sm'>
      <div className='mb-4 text-white'>
        <a href='#' className='mx-2.5'>About</a>
        <a href='#' className='mx-2.5'>Privacy Policy</a>
        <a href='#' className='mx-2.5'>Terms of Services</a>
      </div>
    <p className='text-white'>© Copyright Reserved 2023</p>
    </div>
    </div>
  )
  //https://images.pexels.com/photos/2887718/pexels-photo-2887718.jpeg?auto=compress&cs=tinysrgb&w=300
}

export default Portfolio1
